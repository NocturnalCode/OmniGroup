...
NSString *OFDescribeKevent(const struct kevent *ev)
{
    switch(ev->filter) {
        case EVFILT_PROC:
            ;
            NSString *s = [NSString stringWithFormat:@"filter=PROC pid=%ld flags=%04x %04x", ev->ident, ev->flags, ev->fflags];
            if (ev->data)
                s = [s stringByAppendingFormat:@" data=%ld", (long)(ev->data)];
            return s;
        case EVFILT_READ:
            return [NSString stringWithFormat:@"filter=READ fd=%ld flags=%04x %04x data=%ld", ev->ident, ev->flags, ev->fflags, (long)(ev->data)];
        case EVFILT_WRITE:
            return [NSString stringWithFormat:@"filter=WRITE fd=%ld flags=%04x %04x data=%ld", ev->ident, ev->flags, ev->fflags, (long)(ev->data)];
        default:
            return [NSString stringWithFormat:@"filter=%d ident=%ld flags=%04x %04x data=%ld", ev->filter, ev->ident, ev->flags, ev->fflags, (long)(ev->data)];
    }
}

...
- (NSString *)description;
{
    ...
    [result appendFormat:@"Match:%ld-%ld%c", matchRange.location, NSMaxRange(matchRange)-1, subexpressionCount ? '(' : ' '];
    for (subexpressionIndex = 0; subexpressionIndex < subexpressionCount; subexpressionIndex++) {
        [result appendFormat:@"%ld-%ld%c", subExpressionMatches[subexpressionIndex].location, NSMaxRange(subExpressionMatches[subexpressionIndex]) - 1, subexpressionIndex == subexpressionCount - 1 ? ')' : ','];
    }
    return result;
}
